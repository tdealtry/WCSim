...
//  TFile *f2 = new TFile("output_from_txt.root","READ");
#else
  ...
  /*
  TTree * txtfile_primary_events_tree = (TTree*)f2->Get("primary_events_tree");
  std::vector<std::vector<Int_t> > * txtfile_digitized_hit_tube_id = 0;
  std::vector<std::vector<Float_t> > * txtfile_digitized_hit_time = 0;
  std::vector<std::vector<Float_t> > * txtfile_corrected_hit_time = 0;
  txtfile_primary_events_tree->SetBranchAddress("digitized_hit_tube_id",&txtfile_digitized_hit_tube_id);
  txtfile_primary_events_tree->SetBranchAddress("digitized_hit_time",&txtfile_digitized_hit_time);
  txtfile_primary_events_tree->SetBranchAddress("corrected_hit_time",&txtfile_corrected_hit_time);
  txtfile_primary_events_tree->GetEvent(ievent); 
  */
...
//  int txtfile_number_of_digitized_cherenkov_hits = (txtfile_digitized_hit_tube_id->at(itrigger)).size();
  ...
  /*
  // get time binning
  double txtfile_time_min = FLT_MAX;
  double txtfile_time_max = -FLT_MAX;
  for(int idigitizedhit=0; idigitizedhit<txtfile_number_of_digitized_cherenkov_hits; idigitizedhit++){

    double local_time = (txtfile_corrected_hit_time->at(itrigger)).at(idigitizedhit);
    if( local_time < txtfile_time_min ) txtfile_time_min = local_time;
    if( local_time > txtfile_time_max ) txtfile_time_max = local_time;
  }
  int txtfile_nbins_time = (int)(((int)(txtfile_time_max+1.) - (int)txtfile_time_min)/time_bin_size) + 1;
  std::clog << " txtfile_time min = " << txtfile_time_min << ", txtfile_time max = " << txtfile_time_max << " nbins time = " << txtfile_nbins_time << std::endl;
  */
  ...
  TH1F h_n_pmts_vs_time_background("h_n_pmts_vs_time_background","n_pmts_vs_time_background;time (ns);number of pmts",nbins_time,time_min, time_max);
  h_n_pmts_vs_time_background.SetLineColor(kBlue);
  h_n_pmts_vs_time_background.SetFillColor(kBlue);
  TH1F h_n_pmts_vs_time_signal("h_n_pmts_vs_time_signal","n_pmts_vs_time_signal;time (ns);number of pmts",nbins_time,time_min, time_max);
  h_n_pmts_vs_time_signal.SetLineColor(kRed);
  h_n_pmts_vs_time_signal.SetFillColor(kRed);
  /*
  TH1F h_n_pmts_vs_corrected_time_background("h_n_pmts_vs_corrected_time_background","n_pmts_vs_corrected_time_background;time (ns);number of pmts",txtfile_nbins_time,txtfile_time_min, txtfile_time_max);
  h_n_pmts_vs_corrected_time_background.SetLineColor(kBlue);
  h_n_pmts_vs_corrected_time_background.SetFillColor(kBlue);
  TH1F h_n_pmts_vs_corrected_time_signal("h_n_pmts_vs_corrected_time_signal","n_pmts_vs_corrected_time_signal;time (ns);number of pmts",txtfile_nbins_time,txtfile_time_min, txtfile_time_max);
  h_n_pmts_vs_corrected_time_signal.SetLineColor(kRed);
  h_n_pmts_vs_corrected_time_signal.SetFillColor(kRed);
  */
  ...
  if( make_animation ){
    for(int i=0; i<nbins_time; i++){
      std::clog << " time " << i << " of " << nbins_time << std::endl;
  ...
  for(int idigitizedhit=0; idigitizedhit<number_of_digitized_cherenkov_hits; idigitizedhit++){
    ...
    /*
    int corrected_timebin;
    for(int jdigitizedhit=0; jdigitizedhit<txtfile_number_of_digitized_cherenkov_hits; jdigitizedhit++){
      int txtfile_tube_id = (txtfile_digitized_hit_tube_id->at(itrigger)).at(jdigitizedhit);
      double txtfile_t0 = (txtfile_digitized_hit_time->at(itrigger)).at(jdigitizedhit);
      double txtfile_cort0 = (txtfile_corrected_hit_time->at(itrigger)).at(jdigitizedhit);
      if( txtfile_tube_id == tube_id && txtfile_t0 == t0 ){
	corrected_timebin = (int)((txtfile_cort0 - txtfile_time_min)/time_bin_size);
	break;
      }
    }
    */
    ...
    if( is_background ){
      h_n_pmts_vs_time_background.AddBinContent(timebin);
//      h_n_pmts_vs_corrected_time_background.AddBinContent(corrected_timebin);
    }
    else{
      h_n_pmts_vs_time_signal.AddBinContent(timebin);
//      h_n_pmts_vs_corrected_time_signal.AddBinContent(corrected_timebin);
    }
  ...
  h_n_pmts_vs_time_background.Write();
  h_n_pmts_vs_time_signal.Write();
//  h_n_pmts_vs_corrected_time_background.Write();
//  h_n_pmts_vs_corrected_time_signal.Write();
  h_npe.Write();
