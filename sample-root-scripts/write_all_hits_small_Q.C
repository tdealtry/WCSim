...
int main(){
  ...
  std::vector<double> highQ_times;

  double remove_time = 0.;
  double remove_time_tmp = 0.;
  double end_time = 0.;
  bool write_this;
  ...
  for(int ievent=0; ievent<primary_events_tree->GetEntries(); ievent++){
	...
	charge = (digitized_hit_Q->at(itrigger)).at(idigitizedhit);
	if( charge > Qmax ) highQ_times.push_back(time);
	
      }

      std::sort (highQ_times.begin(), highQ_times.end());
      end_time = 0.;
      for(std::vector<double>::const_iterator it = highQ_times.begin(); it != highQ_times.end(); it++){
	remove_time_tmp += std::min(Tmax, *it + Tmax - end_time);
	end_time = *it + Tmax;
      }
      std::clog << " remove time " << remove_time_tmp << " ns " << std::endl;
      remove_time += remove_time_tmp;
  ...
  std::clog << " total remove time " << remove_time << " ns = " << remove_time*1.e-9 << " s " << std::endl;
