  ...
  :WCSimWCTriggerBase(name, myDetector, myMessenger)
{
  ...
  GetVariables();
  if(!multiDigitsPerTrigger) {
    G4cerr << "multiDigitsPerTrigger is set to false in WCSimWCTrigger" << triggerClassName
	   << "... Turning on and continuing" << G4endl;
    multiDigitsPerTrigger = true;
  }
  if(saveFailuresMode != 0) {
    G4cerr << "saveFailuresMode is set to " << saveFailuresMode << " in WCSimWCTrigger" << triggerClassName
	   << "... Setting to 0 and continuing" << G4endl;
    saveFailuresMode = 0;
  }
}
...
void WCSimWCTriggerNoTrigger::DoTheWork(WCSimWCDigitsCollection* WCDCPMT)
{
  ...
  WriteOutTxtNova(WCDCPMT);
  AlgNoTrigger(WCDCPMT);
}

void WCSimWCTriggerNoTrigger::WriteOutTxtNova(WCSimWCDigitsCollection* WCDCPMT)
{
